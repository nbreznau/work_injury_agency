---
title: "Technical Appendix 1: Data Setup"
output: html_notebook
---

Nate Breznau, breznau.nate@gmail.com

Code for the chapter "The Introduction and Expansion of Work-Injury Law and National Risk Pooling: Testing the Global Role of Worker Agency"
...
*data_legal_slavery_country.csv* (Rosling 2020)
...
workinj<- read.csv("data/gwip_v1.csv", header = T)

```

### GDP

[Maddison 2018 version](http://www.ggdc.net/maddison/maddison-project/home.htm). (Bolt, Inklaar, de Jong and Luiten van Zanden 2018)

To account for missing data the following changes were made to the file *mpd2018.xlsx* 

1. Included missing cells to complete time series for all countries (adjusted xlsx file, just added year rows)
...
*Bhutan* - take Sri Lanka
*Belize* - take Jamaica
*Liechtenstein* and *Monaco*- take Luxembourg
*South Sudan* - take Sudan
*San Marino* & *Andorra* - take Austria

##### Lower GDP Islands, mostly S.E. Asia/Pacific

*Suriname* - substituted with St. Lucia, used 1 for 1900 (adj for crash after 2013)
*Solomon Islands* - tracks Cambodia
*Fiji, Guyana, Samoa, Marshall Islands, Palau, Micronesia, Vanuatu* - track Indonesia back to 1950, then 500 for 1900
*Kiribati* - like Myanmar (start at 500)
...
*Bahamas* and *Brunei* (Brunei 500 in 1900) - 1945 manufacturing takes off, but strong pre-industrial economy; 1850 - 1000; after it is roughly double Barbados (note, 1994-2001 removed, and 2013-2016 removed due to peculiarities in the Maddison data; 2016 replaced with 35000)

*St. Kitts & Nevis* and *Antigua & Barbuda* - like Trinidad & Tobago
*St. Vincent & Grenadines* and *Grenada*- like Dominican Republic
*Papua New Guinea* - like Honduras
...
3. Countries with Missing Historical Data

Scores Added for Interpolation
...
*Estonia*,  *Kazakhstan* (1946 = 4700), *Kyrgystan*, *Latvia*, *Lithuania*, *Tajikistan*, *Turkmenistan* 1950 = 5000
...
*Belarus*, *Ukraine*, and TransCaucasia (*Armenia*, *Azerbaijan*, *Georgia*) (USSR GDP until 1958 to give a transition until 1973, 1st year of Maddison)
...
abolish <- read.csv("data/data_legal_slavery_country.csv", header=T, check.names=F, stringsAsFactors = F)
...
no <- c(20,155,344,349,352,366,367,368,371,372,373,600,616,620,630,640,645,652,660,663,645,679,690,692,694,696,703,760,771,775,780,811,812,817,820,830,835,840,850,920, 200, 205, 210, 211, 212, 220, 221, 223, 225, 230, 235, 255, 290, 305, 310, 316, 317, 325, 338, 339, 345, 350, 355, 359, 360, 365, 369, 370, 375, 380, 385, 390, 395, 651, 666, 698, 700, 702, 704, 705, 710, 712, 731, 732, 740, 750, 770, 800, 817, 940, 950, 983, 986, 987)
...
This chunk formerly produced an unused figure. It might be unnecessary, check at some point.

```{r clean}
#Goal: to identify countries that had some form of work-injury law prior to 1923 (after Soviet Union formation)
#To identify which of those countries experienced some form of industrialization- we do this by taking the GDP of Germany in 1830 # first steam powered railroad in Germany 1835. The interpolated GDP is 1109, but we go for 999 as crude cutoff. At this value we expect railroads and some kind of industrial production to be beginning, i.e., that workers are increasingly experiencing dangerous working conditions


#Ethiopia, Saudi Arabia and Venezuela have NA for firstins, set to past 1923
wi_df$labor_workinjury_firstins <- ifelse(wi_df$cow_code==670 | wi_df$cow_code==530 | wi_df$cow_code==101,1980,wi_df$labor_workinjury_firstins)


wi_df$indep20 <- ifelse(wi_df$independence<1923,1,0)
wi_df$insur20 <- ifelse(wi_df$labor_workinjury_firstins<1923,1,0)
#replace NAs for now (coding wasn't fully finished at this point)
wi_df$insur20 <- ifelse(is.na(wi_df$insur20),1,wi_df$insur20)
wi_df$first20 <- ifelse(wi_df$labor_workinjury_firstlaw<1923,1,0)


# Countries without insurance get coded 2018
wi_df$labor_workinjury_firstins <- ifelse(is.na(wi_df$labor_workinjury_firstins),2018,wi_df$labor_workinjury_firstins)

#Liechtenstein missing GDP in Maddison data, assume industrial revolution
wi_df$gdpi <- ifelse(wi_df$cow_code==223,2100,wi_df$gdpi)


wi_df$indus20 <- ifelse(wi_df$gdpi>999,1,0)
wi_df$first20indus <- ifelse(wi_df$labor_workinjury_firstlaw<1923 & wi_df$indus20==1,1,0)
wi_df$insur20indus <- ifelse(wi_df$labor_workinjury_firstins<1923 & wi_df$indus20==1,1,0)

```

QoQ Basic Data
http://www.qog.pol.gu.se doi:10.18157/qogbasjan20
qog_bas_cs_jan20.csv
...
I add Russian 'colonies' (prior to 1922):

11. Russia


```{r colonialism}
colony <- read.csv("data/qog_bas_cs_jan20.csv", header=T, stringsAsFactors = F)
...
PolityData <- readRDS("data/PolityData.rds")
...
#### Missing (code based on Freedom House after indep, 2019 scores)
31 Bahamas 8 (9/10)
...
223	Liechtenstein take Germany
221	Monaco take France
331	San Marino France
232	Andorra France
...
#### Countries under British rule where the Europeans took over (score 2 prior to indep.)
20 Canada
660 Lebanon - somewhere in between - code 0 prior to 1943 (maybe remove fcolony=1?)
900 Australia


#### Parts of Yugoslavia (recode 1918-2005)
341 Montenegro

#### Parts of the USSR - use Russian scores 1922-1990 (all) 1918-1921 (only if missing)
316 Czech Rep
343 Macedonia
359 Moldova
366 Estonia
...
#### Slavery Adjustments to Rosling data
...
Not used, but potentially useful for future analyses
...
```{r fig1}
agg_png(file = "results/Fig2.png", width = 800, height = 550, units = 'px')
...
legend(x=1975, y=60, title="Work-Injury Law Type", title.adj = 0.2, c('No Law', 'Employer Liability', 'Risk Pooling', 'Social Insurance'), fill=c('mediumseagreen', 'yellowgreen', 'darkorange3','indianred4'), cex=1.2)
dev.off()

knitr::include_graphics("results/Fig2.png")
```
#### Former Communist & Final Recodes
```{r finalrec, warning=F, message=F}
# former/current communist list
fmrc <- read.csv("data/fmrcmnst.csv", header = T)
...
wi_df_long <- wi_df_long %>%
  mutate(gdpD7 = gdp_7yr_rate,
         ...
         # truncate at 1981
         abol = ifelse(abol>1980,1981,abol),
         ...
         noslave = ifelse(cow_code == 817, 0, noslave))
...
```{r savepoint}
# GDP for 
write.csv(wi_df, file = "data/wi_df.csv")
write.csv(wi_df_long, file = "data/wi_df_long.csv")
```
