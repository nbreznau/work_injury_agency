...
wi_map %>%
  mutate(abol = ifelse(noslave == 1, NA, abol),
         abol = ifelse(abol < 1879, 1879, abol)) %>%
  ggplot() +
  geom_sf(aes(fill = abol), size = 0.15, colour = "black") +
  scale_fill_continuous(limits = range(wi_map$abol),
                        breaks = c(1880,1900,1920,1940,1960,1980),
                        labels = c("1880-","1900","1920","1940","1960","1980+"),
                        type = "viridis",
                        direction = -1,
  ...
  coord_sf(label_axes = "----") +
  labs(fill = NULL,
       title = "Abolition Year for Slave Trading/Production Countries",
       caption = "Note: Year when slavery and forced production labor legally ended\nSource: GWIP v1.0 (Breznau and Lanver 2020); Legal Slavery v1 (Rosling 2018) adapted by Author") +
  theme_classic() +
  theme(legend.position = "bottom",
        legend.direction = "horizontal",
        legend.key.width = unit(2, "line"))
...
agg_png(filename = here::here("results","map5.png"), width = 1000, height = 800, res = 144)
wi_map %>%
  ggplot(aes(fill = factor(commst))) +
  geom_sf(size = 0.15, colour = "black") +
  ...
  coord_sf(label_axes = "----") +
  labs(fill = NULL,
       title = "Countries with Communist/socialist Government",
       caption = "Note: excludes temporary coups and dictatorships") +
  theme_classic() +
...
knitr::include_graphics(here::here("results","map5.png"))
```
### Models 
